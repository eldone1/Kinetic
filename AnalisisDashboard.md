# Dashboard — Estado del Módulo

> Progreso: **50%** — Dashboard implementado (frontend + backend APIs). Reportes (PDF/Excel) pendientes.

## Backend
- ✅ `DashboardController` — 3 endpoints: `/api/dashboard/admin`, `/api/dashboard/doctor`, `/api/dashboard/recepcion`
- ✅ `DashboardService` + `DashboardServiceImpl` — agregación de KPIs por rol
- ✅ DTOs: `DashboardAdminDTO`, `DashboardDoctorDTO`, `DashboardRecepcionDTO`, `ChartDataPoint`
- ✅ Métodos de conteo agregados a repositorios existentes

## Frontend
- ✅ `chart.js` + `ng2-charts` instalados
- ✅ `DashboardService` — consume los 3 endpoints
- ✅ `DashboardAdminComponent` — KPIs financieros/operativos + gráficos (ventas mensuales, pacientes/día, servicios, doctores)
- ✅ `DashboardDoctorComponent` — KPIs clínicos + agenda del día + atenciones/día + distribución tratamientos
- ✅ `DashboardRecepcionComponent` — KPIs agenda/caja + agenda del día + cobros pendientes + últimas ventas + resumen caja + gráficos
- ✅ `DashboardWrapperComponent` — renderiza el componente según el rol
- ✅ Ruta `/dashboard` registrada y redirección por defecto cambiada a `/dashboard`
- ✅ `provideCharts(withDefaultRegisterables())` en app.config.ts

## Pendiente (50% restante)
- ⏳ Reportes PDF exportables
- ⏳ Reportes Excel exportables
- ⏳ Filtros avanzados por fechas, doctores, etc.

---

Necesito diseñar dashboards modernos, profesionales y funcionales para un sistema web de gestión de una clínica de rehabilitación física.

La interfaz debe estar pensada para uso diario, con gran cantidad de información, navegación rápida y enfoque en productividad. Debe ser responsive, limpia y con jerarquía visual clara.

Contexto del negocio

La clínica maneja:

Pacientes
Citas médicas
Sesiones de rehabilitación
Historias clínicas
Evolución de pacientes
Doctores y terapeutas
Ventas de servicios
Venta de productos*proximamente*
Inventario*proximamente*
Caja diaria
Reportes administrativos

Existen 3 roles principales:

1. Dashboard Doctor

El doctor necesita visualizar únicamente información clínica.

Debe mostrar:

Tarjetas KPI
Pacientes atendidos hoy
Citas pendientes
Citas completadas
Sesiones programadas hoy
Pacientes en tratamiento activo
Agenda del día

Tabla con:

Hora
Paciente
Tipo de atención
Estado
Próximas citas

Listado rápido.

Pacientes recientes

Últimos pacientes atendidos.

Alertas clínicas
Historias clínicas incompletas
Evoluciones pendientes
Pacientes sin seguimiento
Gráficos
Atenciones por día
Pacientes nuevos vs recurrentes
Distribución de tratamientos
Acciones rápidas
Registrar atención
Completar cita
Registrar evolución
Ver historia clínica

2. Dashboard Recepción

La recepcionista gestiona agenda y cobros.

Debe mostrar:

Tarjetas KPI
Citas del día
Pacientes en espera
Citas confirmadas
Citas canceladas
Ventas del día
Caja actual
Agenda en tiempo real

Tabla con:

Hora
Paciente
Profesional
Estado
Cobros pendientes

Pacientes con servicios pendientes de pago.

Últimas ventas

Listado de ventas recientes.

Resumen de caja
Apertura
Ventas en efectivo
Ventas Yape/Plin
Total acumulado
Alertas*proximamente con productos*
Caja no aperturada
Citas próximas
Acciones rápidas
Agendar cita
Buscar paciente/Cobrar servicio
Aperturar caja
Cerrar caja
Gráficos
Cobros
Métodos de pago
Servicios más vendidos

3. Dashboard Administrador

El administrador necesita una visión completa del negocio.

Debe mostrar:

KPIs financieros
Ventas del día
Ventas del mes
Ingresos por servicios
Ingresos por productos*proximamente*
Ticket promedio
Utilidad estimada
KPIs operativos
Pacientes nuevos
Pacientes activos
Citas programadas
Citas completadas
Sesiones realizadas
KPIs de inventario*proximamente*
Productos con stock bajo*proximamente*
Productos próximos a vencer*proximamente*
Valor del inventario*proximamente*
Productos más vendidos*proximamente*
KPIs de caja
Cajas abiertas
Cajas cerradas hoy
Diferencias de caja
Gráficos
Ventas mensuales
Tendencia de pacientes
Servicios más demandados
Productos más vendidos*proximamente*
Rendimiento por doctor*100%*
Rendimiento por recepcionista*100%*
Alertas importantes
Stock crítico*proximamente*
Productos vencidos*proximamente*
Cajas sin cerrar
Ventas anuladas
Servicios sin cobrar
Acciones rápidas
Gestionar usuarios
Gestionar servicios
Gestionar productos*proximamente*
Gestionar lotes*proximamente*
Ver reportes
Ver historial de cajas
Ver historial de ventas

Requisitos de diseño
Diseño tipo SaaS moderno.
Inspirado en Stripe, Linear, Notion, Clerk, Vercel y sistemas médicos premium.
Uso de tarjetas KPI.
Gráficos claros y profesionales.
Tablas modernas con filtros.
Colores suaves orientados al sector salud.
Excelente experiencia para escritorio.
Diseñar wireframe detallado de cada dashboard.
Explicar la distribución visual de cada sección y justificar por qué cada métrica es importante para ese rol.